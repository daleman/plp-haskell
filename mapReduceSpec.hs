...
main = hspec $ do
  describe "Utilizando Diccionarios" $ do
    ...
    it "puede obtenerse la definicón de una clave siempre que esté definida" $ do
      ...
      [("Superman",["Fuerte","Rápido","Vuela","Visión Láser"]),("Aquaman",["Nada"]),
      ("Green Arrow",[]),("Flash",["Rápido"]),("Batman",[])] ? "Aquaman"                                          `shouldBe` ["Nada"]
      [("Análisis",False),("Algebra",True),("AlgoI",True),("OrgaI",True),("AlgoII",True),
      ("AlgoIII",True),("OrgaII",True),("SO",True),("IS1",True)] ? "Análisis"                                     `shouldBe` False
    ...
    it "pueden agregarse definiciones sin repetir claves" $ do
      ...
      insertWith (&&) ["AR$","U$S"] False [(["EUR","U$S"],True),(["U$S","AR$"],False),(["AR$","U$S"],True),
      (["U$S","EUR"],True),(["EUR","AR$"],True),(["AR$","EUR","AR$"],False)] ! ["AR$","U$S"]                      `shouldBe` False
      insertWith (&&) ["U$S","AR$"] True [(["EUR","U$S"],True),(["U$S","AR$"],False),(["AR$","U$S"],True),
      (["U$S","EUR"],True),(["EUR","AR$"],True),(["AR$","EUR","AR$"],False)] ! ["U$S","AR$"]                      `shouldBe` False
